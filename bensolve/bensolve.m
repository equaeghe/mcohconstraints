...
if options.info>=1
    if options.dual==0
        disp('*****************************');
        disp('*  Primal Benson algorithm  *');
        disp('*****************************');
        disp(sprintf('-- variables:     %d',n));
        disp(sprintf('-- constraints:   %d',m));
        disp(sprintf('-- objectives:    %d',q));
        disp(sprintf('-- variant:       %c',options.variant));
        disp(sprintf('-- parameter c:   %s',mat2str(c)));
        lps={'0 (MATLAB linprog)','1 (cdd criss-cross)','2 (cdd dual simplex)','3 (glpk revised simplex)'};
        disp(sprintf('-- lp_solver:     %s',lps{options.lp_solver+1}));
        disp(sprintf('-- eps:           %0.e',options.eps));
        disp(sprintf('-- alg3_eps:      %0.e',options.alg3_eps));
        disp(sprintf('-- vert_enum:     variant %c',options.vert_enum));
        disp(sprintf('-- vert_enum_eps: %0.e',options.vert_enum_eps));
    elseif options.dual==1
        disp('***************************');
        disp('*  Dual Benson algorithm  *');
        disp('***************************');
        disp(sprintf('-- variables:     %d',n));
        disp(sprintf('-- constraints:   %d',m));
        disp(sprintf('-- objectives:    %d',q));
...
if options.info>=1
    disp('  alg3: computing interior point of D^* ...');
end    
p = size(Y,2);
...
if options.info>=1
    disp('  done.');
    disp('  alg1: solving homogeneous problem ...');
end
if options.dual==0
...
if options.info>=1
    disp('  done.');
    disp('  alg1: solving inhomogeneous problem ...');
end
...
if options.info>=2 
    disp(sprintf('Vector optimization problem solved. (%d LPs).',lp_count));
end    
lp_count=0;

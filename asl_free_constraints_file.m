...
% ASL_FREE_CONSTRAINTS_FILE  writes out the 'free' constraints for avoiding sure loss
%
% Synopsis:
...
%    μ >= 0, and 1'μ == 1. These are written out to a file in 'Polyhedra H
%    format'. This format can be read by polytpe theory programs such as
%    cddlib and lrs, for example, to compute the P-only H-representation of
%    the set of lower previsions on K that avoid sure loss. Using cddlib,
%    one would, e.g., say (on a Unix command line)
%
